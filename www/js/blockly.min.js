...
Blockly.Workspace.prototype.updateVariableList=function(a){
    console.log("Llamando a actualizacion de variables...");
    if(!this.isFlyout){
        if (a) {
            const usedVars = Blockly.Variables.allUsedVariables(this);
            usedVars.forEach(varName => {
                if (!this.variableList.includes(varName)) {
                    this.createVariable(varName);
                }
            });
        }
    }
    Blockly.Events.fire(new Blockly.Events.Ui(null, 'variable_list_updated', null, null));
...
//En la version anterior podía utilizar ipcRenderer porque incluía el electron remote. A partir de la 2.0, ya no (separo ambos canales)
 // Blockly.prompt=function(a,b,c){c(ipcRenderer.sendSync("modalVar", a))};
Blockly.prompt=function(a,b,c){
    c(window.api.enviar("modalVar", a))};
