...
BlocklyDuino.renderArduinoCodePreview = function() {
	//Esto lo añado yo para poner la variable de verificación de nuevo a false (se supone que el programa ha cambiado)
	...
	reader.onloadend = function(event) {
		...
		if (target.readyState == 2) {
			...
			if (files[0].name.endsWith("bloc")||files[0].name.endsWith("xml")) {
				...
				window.api.enviar("prueba",event.target.result);
...
BlocklyDuino.loadConfig = function() {
	...
	var instalando=window.localStorage.instalando;
	console.log("Verif: "+verif);
	if (verif==undefined){
		window.localStorage.setItem('verif',false);
		window.localStorage.setItem('instalando',false);
	}
	if (instalando==undefined){
		window.localStorage.setItem('instalando',false);
	}
	if (card===undefined) {
...
BlocklyDuino.openConfigToolbox = function() {
	...
	if (loadIds === undefined || loadIds === "") {
		if ($('#defaultCategories').length) {
			console.log("#defaultCategories definida...");
...
BlocklyDuino.changeToolbox = function() {
	...
	window.localStorage.setItem('theme','masaylo');
	Blockly.getMainWorkspace().updateToolbox(BlocklyDuino.buildToolbox());
	BlocklyDuino.workspace.render();
